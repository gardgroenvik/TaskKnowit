...
import { GITREPOS } from './../constants/constants';

import { Link } from 'react-router-dom';

export default function SpecificRepo(props) {
    const [characterResults, setcharacterResults] = useState(undefined);
    const [isLoading, setIsLoading] = useState(true);

    useEffect(() => {
        axios.get(GITREPOS + '/' + props.match.params.id)
            .then(result => {
                setcharacterResults(result.data.items);
                setIsLoading(false)
            })
    }, [props])
